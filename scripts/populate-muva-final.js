...
/**
 * MUVA EMBEDDINGS PROCESSOR - FINAL VERSION
 * Con categorías correctas y chunking optimizado
 */
...
// Mapeo de categorías de español a inglés
...
function mapMetadataToColumns(metadata, chunk, fileName) {
  ...
  // Mapear categoría a inglés
  let category = CATEGORY_MAPPING[businessType] || 'attraction';

  // Si el business_type viene del directorio, usarlo para categorizar mejor
  const dirName = path.dirname(fileName);
  if (dirName.includes('actividad')) {
    category = 'activity';
  } else if (dirName.includes('restaurante')) {
    category = 'restaurant';
  } else if (dirName.includes('night-life')) {
    category = 'nightlife';
  } else if (dirName.includes('alquiler')) {
    category = 'shopping';
  } else if (dirName.includes('spot')) {
    category = 'attraction';
  }

  // Crear descripción desde el contenido si no existe
  let description = metadata.description || '';
  if (!description || description.includes('[') || description.includes('es seguro') || description.includes('las cuales')) {
  ...
  // Agregar tags basados en el tipo de negocio y zona
  tags.push(businessType.toLowerCase());
  tags.push(zone.toLowerCase());
  tags.push(category);

  // Eliminar duplicados y tags vacíos
  tags = [...new Set(tags)].filter(t => t && t.length > 0);

  // Información de contacto
  const contactInfo = {};
  if (metadata.contact) {
    if (metadata.contact.whatsapp && !metadata.contact.whatsapp.includes('[')) {
      contactInfo.whatsapp = metadata.contact.whatsapp;
    }
    if (metadata.contact.email && !metadata.contact.email.includes('[')) {
      contactInfo.email = metadata.contact.email;
    }
    if (metadata.contact.instagram && !metadata.contact.instagram.includes('[')) {
      contactInfo.instagram = metadata.contact.instagram;
    }
    if (metadata.contact.website && !metadata.contact.website.includes('[')) {
      contactInfo.website = metadata.contact.website;
    }
  }

  // Horarios
  let openingHours = null;
  if (metadata.business_hours?.schedule && !metadata.business_hours.schedule.includes('[')) {
    openingHours = metadata.business_hours.schedule;
  }

  // Precio
  let priceRange = null;
  if (metadata.pricing?.range && !metadata.pricing.range.includes('[')) {
...
async function processFile(filePath) {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // Mapear metadata a columnas de BD (pasando ruta completa para mejor categorización)
    ...
    return {
      ...
      stats: {
        ...
        category: embeddings[0]?.category // Para debugging
      }
    };
...
async function main() {
  ...
  console.log(chalk.gray('Features: Category mapping, smart chunking, quality validation\n'));
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (result.success) {
      ...
      try {
        ...
        console.log(chalk.green(`  ✅ Inserted ${inserted} embeddings (category: ${result.stats.category})`));
      } catch (error) {
  ...
  console.log(chalk.blue(`📂 Categories used: ${[...results.categoriesUsed].join(', ')}`));
  ...
  // Verificar calidad de chunking
  console.log(chalk.cyan('\n📝 Checking chunking quality...'));
  ...
  if (samples && samples.length > 0) {
    ...
    samples.forEach((sample, idx) => {
      ...
      const quality = hasWordCut ? '⚠️ Word cut detected' : '✅ Clean boundary';

      console.log(chalk.gray(`\n  ${idx + 1}. [${sample.category}] ${sample.title}`));
      console.log(chalk.gray(`     "${preview}..."`));
      console.log(chalk.gray(`     ${quality}`));
    });
  }
