...
/**
 * Mapea el rango de precios al formato correcto
 */
function getPriceRange(price) {
  ...
  // Intentar detectar el rango basándose en el texto o número
  const priceStr = String(price).toLowerCase();

  // Si contiene números, intentar estimar el rango
  const numbers = priceStr.match(/\d+/g);
  if (numbers && numbers.length > 0) {
    const firstNum = parseInt(numbers[0]);
    if (firstNum < 50) return '$';
    if (firstNum < 150) return '$$';
    if (firstNum < 300) return '$$$';
    return '$$$$';
  }

  // Si contiene palabras clave
  if (priceStr.includes('económico') || priceStr.includes('barato')) return '$';
  if (priceStr.includes('moderado') || priceStr.includes('medio')) return '$$';
  if (priceStr.includes('caro') || priceStr.includes('premium')) return '$$$';
  if (priceStr.includes('lujo') || priceStr.includes('exclusivo')) return '$$$$';

  // Default a rango medio si no se puede determinar
  return '$$';
}
...
async function processFile(filePath) {
  ...
  try {
    ...
    // Extraer metadata
    const businessType = frontmatter.business_type || 'General';
    const categoryEnglish = categoryMap[businessType.toLowerCase()] || 'attraction';
    ...
    const metadata = {
      ...
      business_hours: frontmatter.business_hours || {}
    };

    // Generar ID único base para el documento
...
${mainContent}
    ...
    if (deleteError && deleteError.code !== 'PGRST116') {
      console.log(`  ⚠️  No se encontraron embeddings previos para eliminar`);
    }
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // Preparar datos para insertar según estructura de muva_embeddings
      const embeddingData = {
        ...
        location: metadata.location.zone || 'San Andrés',
        city: 'San Andrés',
        coordinates: null, // Se puede agregar si hay coordenadas disponibles
        rating: metadata.rating || null,
        price_range: getPriceRange(metadata.pricing.range),
        source_file: fileName,
        chunk_index: i,
        total_chunks: chunks.length,
        opening_hours: metadata.business_hours.schedule || null,
        contact_info: {
