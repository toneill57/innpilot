...
/**
 * Convert ISO date (YYYY-MM-DD) to MotoPress format (DD/MM/YYYY)
 *
 * @param isoDate - Date in ISO format (e.g., "2025-11-20")
 * @returns Date in DD/MM/YYYY format (e.g., "20/11/2025")
 */
function formatDateForMotoPress(isoDate: string): string {
  const [year, month, day] = isoDate.split('-')
  return `${day}/${month}/${year}`
}

/**
 * Generate availability URL with intent parameters (MotoPress Hotel Booking format)
 *
 * @param baseURL - Base URL of the website (e.g., "https://simmerdown.house")
 * @param intent - Travel intent with check-in, check-out, guests
 * @returns URL string with query parameters or null if no check-in date
 */
export function generateAvailabilityURL(
  baseURL: string,
  intent: PublicSession['travel_intent']
): string | null {
  const { check_in, check_out, guests } = intent

  // Only generate if we have at least check-in date
  if (!check_in) {
    console.log('[availability-url] No check-in date, skipping URL generation')
    return null
  }

  // Convert ISO dates to MotoPress format
  const checkInMotoPress = formatDateForMotoPress(check_in)
  const checkOutMotoPress = check_out ? formatDateForMotoPress(check_out) : null

  const params = new URLSearchParams()

  // MotoPress uses both DD/MM/YYYY and YYYY-MM-DD formats in same URL
  // First, add DD/MM/YYYY format
  params.append('mphb_check_in_date', checkInMotoPress)
  if (checkOutMotoPress) {
    params.append('mphb_check_out_date', checkOutMotoPress)
  }

  // Add adults and children params
  params.append('mphb_adults', guests ? guests.toString() : '2')
  params.append('mphb_children', '0')

  // Also include ISO format dates (MotoPress accepts both)
  params.append('mphb_check_in_date', check_in)
  if (check_out) {
    params.append('mphb_check_out_date', check_out)
  }

  const url = `${baseURL}/search-results/?${params.toString()}`
  console.log('[availability-url] Generated:', url)
